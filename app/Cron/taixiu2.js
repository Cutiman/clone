			...
			Promise.all(list.map(function(obj){
				var action = new Promise((resolve, reject)=> {
					TaiXiu_User.findOne({uid: obj.uid}, function(error, data) {
						...
						Promise.all(Object.values(io.nsps['/users'].sockets).map(function(client){
							if (client.UID !== void 0 && client.UID == obj.uid){
								io.nsps['/users'].to(client.id).emit('p', {taixiu:{status:{win:obj.win, select:obj.select, bet: bet}}});
							}
						}));
				...
				.then(results => {
					if (results.length) {
						Promise.all(results.map(function(obj){
							var action = new Promise((resolve, reject) => {
								UserInfo.findOne({id: obj.uid}, 'name', function(err, users){
									if (obj.taixiu) {
										resolve('<color=#FFFF00>' + users.name + '</color><color=#FFFFFF> thắng </color><color=#FFFF00>' + Helpers.numberToBet(obj.betwin+obj.bet) + '</color><color=#FFFFFF> game </color><color=#52FF00>Tài Xỉu</color>');
									}else{
										resolve('<color=#FFFF00>' + users.name + '</color><color=#FFFFFF> thắng </color><color=#FFFF00>' + Helpers.numberToBet(obj.betwin+obj.bet) + '</color><color=#FFFFFF> game </color><color=#52FF00>Chẵn Lẻ</color>');
									}
								});
							});
							return action;
						...
						.then(result => {
							Promise.all(Object.values(io.nsps['/users'].sockets).map(function(client){
								if (client.auth){
									io.nsps['/users'].to(client.id).emit('p', {news:{a:result}});
								}
							}));
						})
					}
